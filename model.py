...
if __name__ == "__main__":
    ...
    train_imgs = [
        os.path.join(TRAIN_IMG_DIR, png) for png in os.listdir(TRAIN_IMG_DIR)
    ]
    train_masks = [
        os.path.join(TRAIN_MASK_DIR, png) for png in os.listdir(TRAIN_MASK_DIR)
    ]

    TEST_IMG_DIR = os.path.join(TEST_DIR, "img")
    TEST_MASK_DIR = os.path.join(TEST_DIR, "mask")
    test_imgs = [
        os.path.join(TEST_IMG_DIR, png) for png in os.listdir(TEST_IMG_DIR)
    ]
    test_masks = [
        os.path.join(TEST_MASK_DIR, png) for png in os.listdir(TEST_MASK_DIR)
    ]
    ...
    train_dataset = (
        train_processed_image_ds.cache()
    ...
    CP = ModelCheckpoint(
        filepath=os.path.join(
            LOG_DIR,
            "VGG19-Augmentation-{epoch:03d}-{loss:.4f}-{accuracy:.4f}-{val_loss:.4f}-{val_accuracy:.4f}.hdf5",
        ),
